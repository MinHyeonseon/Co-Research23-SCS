...
%   Copyright 2022 The MathWorks, Inc.
    ...
    methods
        ...
        function [data,timestamp] = capture(obj,length)
            ...
            if isduration(length)
                length = ceil(seconds(length)*obj.SampleRate);
            end

            if matches(obj.DeviceName, obj.ListOfOtherTransceivers)
                [data,mData] = capture(obj.SDRObj,length,'EnableOversizeCapture',true);
                timestamp = datetime(mData.Date);
            else % USRP and RTL-SDR
                data = zeros(length,1,'like',1i);
                samplesPerFrame = obj.SamplesPerFrame;
                if obj.SDRObj.NumFramesInBurst ~= ceil(length/samplesPerFrame)
                    release(obj.SDRObj);
                    obj.SDRObj.NumFramesInBurst = ceil(length/samplesPerFrame);
                end
                timestamp = datetime('now');
                for i = 1:obj.SDRObj.NumFramesInBurst
                    data(samplesPerFrame*(i-1)+(1:samplesPerFrame)) = obj.SDRObj();
                end
            end
        end
    ...
    methods(Access=private)
        ...
        function sdrObj = instantiateSDRObj(obj, deviceName)
            ...
            % Enable burst mode for USRPs or RTL-SDR
            if matches(deviceName,[obj.ListOfUSRPs,'RTL-SDR'])
                sdrObj.EnableBurstMode = true;
                sdrObj.SamplesPerFrame = 10e3;
            end
